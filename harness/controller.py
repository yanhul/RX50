...
def authorized_terminal(state):
 ...
 try:
  verify_terminal_attestation(state,state.get("terminal_attestation"),os.environ.get("RX50_TERMINAL_AUTHORITY_SECRET", "")); print("TERMINAL_AUTHORITY_VERIFIED"); return True
 except Exception as exc: print("TERMINAL_AUTHORITY_HOLD:"+str(exc)); return False
def main()->int:
 ...
 if state.get("terminal"):
  ...
  checkpoint(state,phase="HOLD",result="HOLD: UNAUTHORIZED TERMINAL",last_error="persisted terminal state lacks valid external authority attestation"); return 3
 ...
 if status!="AUDIT_REQUIRED": result="HOLD"; reason="queue item is not in AUDIT_REQUIRED state"
  ...
  iteration=int(state.get("iteration",0))+1; limit=min(MAX_ITERATIONS,int(queue.get("max_iterations",MAX_ITERATIONS))); result="HOLD: ITERATION_LIMIT" if iteration>limit else "HOLD: REGISTERED AUDIT REQUIRED"; reason="iteration limit reached; resume only after new evidence/state change" if iteration>limit else "promotion requires the registered audit path"; state=checkpoint(state,phase="DECIDE",iteration=iteration)
 state=checkpoint(state,phase="VERIFY",result=result,last_error=reason); state=checkpoint(state,phase="PERSIST",result=result); state=checkpoint(state,phase="YIELD",previous_queue_sha256=queue_sha,current_item=item.get("id")); print(state["result"]); print("phase="+state["phase"]); print("queue_sha256="+state["queue_sha256"]); return 0
